...
- Now, we attach an event listner to each displayed book, so that when a User clicks a book, we take the id of that book and pass that id as a prop into the `BookDetails` component. Then we attach that id prop to the query variable and then the details of that book will be shown.

1. Go to `BookList` and attach an event listner(onClick here) to each displayed Book in the list
...
- Then go to `BookDetails` and write:
