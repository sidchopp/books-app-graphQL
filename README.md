...
- NOTE: Now, we want to link a book, that we want to add to our DB, with it's author and for that we have used a property called `authorid`. BUT, now that `_id` is generated by the MongoDB. So, we copy/take the "respective value" of `_id` from each document in `authors` collection in MongoDB, which corresponds to that book we want to add.So, this way we can relate an Author and Book. And then we paste that value as the value of `authorid`, while doing `addBook` mutation. Like this:
  Now in graphiql, try:
